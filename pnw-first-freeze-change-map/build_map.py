"""
Pacific Northwest First Freeze Change Map -- builder
Ingalls Weather
...
change in average first fall freeze date (daily low <=32F / 0C) between
1961-1975 and 2006-2020 (see fetch_climatology.py), plotted as a
diverging-colored dot at that station's location -- blue where the freeze
now arrives earlier than it used to, red where it arrives later. No
interpolation or shading between stations, same reasoning as
../pnw-first-freeze-map/ (see that project's docstring/README): station
coverage is uneven and elevation-driven enough that a smoothed fill would
visually claim more precision than the data supports -- doubly so here,
since this map's station filter (a real record on both ends of a ~60-year
span) is stricter and the candidate pool smaller than the sibling map's.
...
# ---------------------------------------------------------------------------
# Change color table -- fixed control points keyed to the change in days
# (recent-period mean offset minus early-period mean offset), not rescaled
# per map, diverging around 0: blue where the average first freeze now
# arrives earlier than 1961-1975 (a shorter frost-free season), white/gray
# near no change, red where it now arrives later (a longer frost-free
# season -- the more commonly expected direction under a warming climate).
# +-35 days chosen from the actual fetched distribution (kept-station
# mean +10.2d, median +9.0d, 5th/95th percentile about -4.6d/+31.5d, a
# handful of individual-station outliers out to -64d/+77d that this range
# deliberately clips to the table's end colors rather than stretching the
# whole scale to accommodate a few thin/noisy records).
# ---------------------------------------------------------------------------
...
def build_change_colormap():
    ...
    return LinearSegmentedColormap.from_list("ingalls_first_freeze_change", stops, N=256)
...
def build_map(climatology, output_path):
    ...
    # in mean first-freeze date -- no interpolation/shading between
    ...
    fig.text(0.03, 0.977, "Pacific Northwest Change in Average First Freeze", fontsize=17.5,
    ...
    fig.text(0.03, 0.940, f"{climatology['period_early']} to {climatology['period_recent']} "
                          f"• Change in First Fall Date ≤32°F (0°C)",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build an Ingalls Weather PNW/BC first-freeze change map.")
    ...
    parser.add_argument("--out", type=Path, default=None,
                         help="Output PNG path (default: output/pnw_first_freeze_change_map.png).")
    ...
    out_path = args.out or (OUTPUT_DIR / "pnw_first_freeze_change_map.png")
