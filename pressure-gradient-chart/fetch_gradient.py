"""
Fetches a full local calendar day's near-real-time observations for two NWS
ASOS/AWOS stations and writes gradient_obs.json: a single time series of
station_a_pressure_mb - station_b_pressure_mb (the "gradient"), paired with
each station's own reading at that time.
...
a User-Agent identifying the requester.
...
def fetch_observations(station_id, start_utc, end_utc):
    ...
    r = requests.get(f"{BASE_URL}/stations/{station_id}/observations",
...
def parse_args():
    ...
    ap.add_argument("--timezone", default="America/Los_Angeles",
                     help="Local calendar day both stations are treated as sharing (default: America/Los_Angeles)")
    ap.add_argument("--date", default=None,
                     help="YYYY-MM-DD, local calendar day (default: today, local)")
...
if __name__ == "__main__":
    ...
    end_utc = min(day_end, datetime.now(tz)).astimezone(timezone.utc) + timedelta(minutes=1)
    ...
    if observations:
        print(f"Saved {args.output}: {len(observations)} paired obs for "
              f"{args.label_a}-{args.label_b} on {target_date} "
              f"({observations[0]['time']} .. {observations[-1]['time']})")
    else:
        print(f"Saved {args.output}: 0 paired observations for {args.label_a}-{args.label_b} on {target_date}")
