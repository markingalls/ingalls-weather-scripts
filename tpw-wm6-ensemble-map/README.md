...
Western Canada in one frame. Rendered under PlateCarree so data and
basemap lines fill the frame edge-to-edge; a curved satellite-view
projection was tried, but at this domain's width meridians converge
enough toward the pole that the visible area becomes a trapezoid no
projection choice can fit into a rectangle without leaving the corners
blank (see build_map()'s comment on projection choice). The color table
...
level and would otherwise look blocky (a separate step from the crop/
upsample, which is why `scipy` is a dependency -- see `requirements.txt`).
