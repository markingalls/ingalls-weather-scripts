...
Surface pressure centers get "L"/"H" markers (red/blue) via
`find_pressure_extrema()`: a sliding min/max filter finds local extrema in
the native-resolution MSLP grid, then a prominence check against a much
wider local-mean window keeps only extrema that clear a full
`MSLP_CONTOUR_INTERVAL_HPA` -- i.e. extreme enough that a drawn isobar
could plausibly close a loop around them. Without that filter, this
field's small terrain-driven MSLP-reduction ripples over the Great
