...
REGIONS = {
    ...
    "offshore_pnw": dict(
        ...
        output="offshore_pnw_lightning.png",
        cities=[
...
def _draw_static_layers(ax, pc, cfg):
    """Land, countries, states+lakes, counties, roads -- every layer that
    depends only on the region's own config, never on the polled lightning
    data. This is the expensive part (tens of thousands of road line
    segments alone), which is exactly why it's the part that gets cached
    rather than redrawn on every run."""
    # ---------- land ----------
