...
(highways). The WA/OR files originally were pre-clipped to a bounding box
that stopped short of this map's widened western edge and cities like
Portland, Salem, Longview, and Olympia; a supplemental OSM motorway/trunk
extract (via the Overpass API) was merged into them (de-duplicated against
what was already there) rather than kept as a separate file. The Ingalls
