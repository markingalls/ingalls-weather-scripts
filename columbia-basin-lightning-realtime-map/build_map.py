...
REGIONS = {
    "columbia_basin": dict(
        ...
        output="columbia_basin_lightning_realtime.png",
        cities=[
    ...
    "portland": dict(
        ...
        output="portland_lightning_realtime.png",
        cities=[
    ...
    # british_columbia_roads.geojson is included even though this domain
    # only just crosses the border, for the sliver of BC that is in frame.
    "pnw": dict(
    ...
    "offshore_pnw": dict(
        ...
        output="offshore_pnw_lightning_realtime.png",
        cities=[
    ...
    "full_bc": dict(
        ...
        output="full_bc_lightning_realtime.png",
        cities=[
    ...
    "puget_sound": dict(
        ...
        output="puget_sound_lightning_realtime.png",
        cities=[
...
def build_map(region_key, lightning_path, output_path):
    ...
    # BC no longer observes standard time, so the BC regions (bc_interior,
    # full_bc, lower_mainland_victoria -- each with an explicit "timezone"
    # override) always label themselves "PDT" -- not computed from the
    # zone, since zoneinfo's America/Vancouver rules still assume a DST
    # fallback that no longer happens. Every other region sits on the US
    # side (America/Los_Angeles, still observes standard time) and gets a
    # real PDT/PST label computed from that zone.
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Render a lightning (last 2 hours) map for one region.")
    ap.add_argument("--region", choices=sorted(REGIONS), default="columbia_basin")
    ap.add_argument("--lightning", default="lightning_last2h.json")
