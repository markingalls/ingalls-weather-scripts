...
# ---------- recency bands ----------
# (max_age_hours, label, color) -- newest first. Same palette as
# ../columbia-basin-lightning-map (24h version), just compressed onto a
# 2-hour real-time window instead of a full day. Drawn oldest-first so
# recent strikes sit on top.
AGE_BANDS = [
...
# ---------- lightning flashes ----------
data = json.load(open("lightning_last2h.json"))
...
fig.text(left_x, title_y, "Columbia Basin: Lightning (Last 2 Hours)",
...
plt.savefig("columbia_basin_lightning_realtime.png",
