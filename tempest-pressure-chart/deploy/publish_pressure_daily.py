...
published as hermiston_pressure_day{1..5}.png -- a look-back companion to
the always-current hermiston_pressure.png publish_pressure.py maintains
every 5 minutes.

Same rotation/idempotency logic as
tempest-temp-chart/deploy/publish_daily.py (itself modeled on
columbia-basin-lightning-daily-map/deploy/publish_daily.py's nightly
refresh) -- see that file's own docstring for the full reasoning on why
this runs hourly rather than once at a fixed time (idempotent per Pacific
calendar date, tracked in state/last_rotation_date.txt) and why rotation
processes i descending from 5.

Renders with no_current_conditions=True (day1 is a complete, past day,
not still in progress -- there's no "current" reading to highlight, though
the high/low markers still show).

Fetches into tempest_pressure_obs_daily.json, deliberately not the plain
tempest_pressure_obs.json publish_pressure.py's own every-5-minutes tick
reads and writes -- these two cron jobs run independently (separate lock
files) and could tick concurrently, so sharing one file would risk one
script reading the other's half-written data.

An flock-based lock (state/daily_run.lock, distinct from
publish_pressure.py's state/run.lock) means an overlapping cron tick (e.g.
a slow run still in progress when the next scheduled tick fires) skips
instead of running a second pass concurrently -- and doesn't contend with
publish_pressure.py's own every-5-minutes lock either.
"""
import fcntl
import os
import subprocess
...
N_DAYS = 5
OUTPUT_BASE = "hermiston_pressure"
...
def publish_day1(obs_path):
    ...
    build_chart.build_chart(obs_path, tmp_path, no_current_conditions=True)
...
def main():
    ...
    try:
        ...
        obs_path = os.path.join(BASE_DIR, "tempest_pressure_obs_daily.json")
