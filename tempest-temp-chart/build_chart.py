...
def build_chart(data_path, forecast_path, output_path, mark_low=False, mark_high=False):
    ...
    if os.path.exists(forecast_path):
        fdata = json.load(open(forecast_path))
        if fdata.get("date") == data["date"]:
            forecast_high = fdata.get("forecast_high_f")
        else:
            print(f"NOTE: {forecast_path} is for {fdata.get('date')}, not {data['date']} "
                  f"-- ignoring it for axis scaling.")
    else:
        print(f"NOTE: no forecast file at {forecast_path} -- axis scaled to observed data only. "
    ...
    if times and (mark_low or mark_high):
