...
below). On the droplet, `deploy/publish_daily.py` maintains this as an
actual published 5-day archive going forward, one day at a time via
nightly rotation -- see "Deployment" below.
...
- `build_chart.py` -- renders `tempest_obs.json` (+ `forecast.json`, if
  ...
  mark_low=False, mark_high=False, no_current_conditions=False)` function
  -- `main()` is a thin CLI wrapper around it -- so `deploy/publish_tempest.py`
  and `deploy/publish_daily.py` can both call it directly rather than
  shelling out.
...
- `deploy/publish_tempest.py` -- cron entry point. Fetches Tempest
  ...
  "Deployment" below.
- `deploy/publish_daily.py` -- cron entry point. Once a day (idempotent
  per Pacific calendar date, so safe to run hourly), rotates and
  republishes the `hermiston_temp_day{1..5}.png` 5-day archive. See
  "Deployment" below.
- `deploy/crontab.example` -- `publish_tempest.py` every 5 minutes,
  `publish_daily.py` hourly, with `TEMPEST_API_KEY` set inline (cron
...
- **Dew point line**: plotted alongside temperature (forest green,
  ...
  above), but deliberately has no `--mark-low`/`--mark-high` equivalent --
  those stay temperature-only. Missing-RH observations (rare) come back
  from `fetch_tempest.py` with no `dew_point_f` key at all;
  `build_chart.py` substitutes `NaN` for those before plotting, so a lone
  missing reading breaks the line the same way a real outage does, rather
  than crashing or silently interpolating across it.
...
- **`--no-current-conditions`** renders a plain historical-day chart: no
  ...
  and `deploy/publish_daily.py` both always pass this (plus `--mark-low
  --mark-high` / `mark_low=True, mark_high=True`, since a full day's
...
- `https://images.ingallswx.com/hermiston_temp.png` -- today, refreshed
  every 5 minutes.
- `https://images.ingallswx.com/hermiston_temp_day1.png` through
...
real `TEMPEST_API_KEY` in place of the placeholder -- both
`publish_tempest.py` (every 5 minutes) and `publish_daily.py` (hourly,
idempotent per Pacific calendar date -- see its own docstring and
`deploy/crontab.example`'s comment for why hourly-at-:10-past reliably
lands right at 00:10 PT year-round despite DST, without needing a
DST-aware cron time).
...
```bash
venv/bin/python3 deploy/publish_daily.py
tail -f state/publish.log
```

This populates just `hermiston_temp_day1.png` (yesterday) on a first
run -- the rotation loop finds nothing yet in `day1`-`day4` to shift up,
same as `columbia-basin-lightning-daily-map`'s own nightly refresh. The
rest of the archive (`day2`-`day5`) fills in naturally over the next 4
nights as `publish_daily.py`'s hourly cron tick does its once-a-day
rotation. If you want the full 5-day archive live immediately instead of
waiting, run `venv/bin/python3 build_lookback_charts.py` once by hand and
copy its 5 output PNGs from `lookback/` into `/var/www/images/` as
`hermiston_temp_day1.png` (newest) through `hermiston_temp_day5.png`
(oldest) -- after that, `publish_daily.py`'s nightly rotation takes over
normally, sliding the whole archive forward each night the same way it
always does.
...
`image-hit-stats/update_stats.py`'s `CATEGORY_PATTERNS` already covers
both the current and look-back images with one pattern
(`hermiston_temp(_day[1-5])?\.png` -> "Tempest Temp Chart").
