"""
Pacific Northwest Frost-Free Period Map -- builder
Ingalls Weather

Styled map covering the same BC/WA/OR/ID (+ slivers of NV/MT/WY) domain as
../pnw-first-freeze-map/ and ../dew-point-storm-map/: each station's
1991-2020 average number of frost-free days -- the span between the last
spring date and the first fall date its daily low temperature drops to
<=32F (0C) (see fetch_climatology.py) -- plotted as a colored dot at that
station's location, short-to-long on a sequential scale. No interpolation
or shading between stations, same reasoning as ../pnw-first-freeze-map/
(see that project's docstring/README): station coverage is uneven and
...
# ---------------------------------------------------------------------------
# Map domain -- identical to ../pnw-first-freeze-map/'s and
# ../dew-point-storm-map/'s BC/WA/OR/ID (+ slivers of NV/MT/WY) framing.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Frost-free-days color table -- fixed control points keyed to day count
# (not rescaled per map, so a given shade always means the same length
# season across runs), sequential short-to-long: purple/blue (a short
# season -- high mountain interior) through teal/green (a moderate
# season) to gold (a long season -- milder low-elevation and coastal
...
def build_days_colormap():
    span = DAYS_MAX - DAYS_MIN
    stops = [((d - DAYS_MIN) / span, [c / 255 for c in rgb]) for d, rgb in DAYS_COLOR_TABLE]
    return LinearSegmentedColormap.from_list("ingalls_frost_free_days", stops, N=256)
...
def build_map(climatology, output_path):
    ...
    days = np.array([s["mean_frost_free_days"] for s in stations])
    ...
    cmap = build_days_colormap()
    norm = Normalize(vmin=DAYS_MIN, vmax=DAYS_MAX)
    ...
    # frost-free day count -- no interpolation/shading between stations
    # (see module docstring). A white halo underneath the colored fill
    # (drawn as a larger white-edge marker first, kept fully opaque)
    # keeps every dot legible against both the light land fill and darker
    # colors in the table; the colored fill itself gets alpha=0.4, same
    # as ../pnw-first-freeze-map/, so overlapping dots in dense clusters
    # still show through each other.
    ax.scatter(lons, lats, s=95, facecolor="none", edgecolor="white", linewidth=2.2,
               transform=pc, zorder=3.9)
    ax.scatter(lons, lats, c=days, cmap=cmap, norm=norm, s=70, edgecolor="black",
    ...
    # labeled as day counts. alpha=0.4 matches the station dots above, so
    # the swatch shows the same color a single dot actually renders at,
    # not a stronger one.
    fig.canvas.draw()
    ...
    gradient = np.linspace(DAYS_MIN, DAYS_MAX, 256).reshape(1, -1)
    cax = fig.add_axes([cbar_left, cbar_bottom, cbar_width, cbar_height])
    cax.set_facecolor("white")
    cax.imshow(gradient, aspect="auto", cmap=cmap, norm=norm, alpha=0.4,
               extent=[DAYS_MIN, DAYS_MAX, 0, 1])
    cax.set_yticks([])
    ...
    fig.text(0.03, 0.977, "Pacific Northwest Frost-Free Period", fontsize=19,
              fontproperties=poppins_reg, color="#2b2a26", ha="left", va="top")
    fig.text(0.03, 0.940, f"{climatology['period']} Climatology • Days Between Freezes ≤32°F (0°C)",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build an Ingalls Weather PNW/BC frost-free period map.")
    ...
    parser.add_argument("--out", type=Path, default=None,
                         help="Output PNG path (default: output/pnw_frost_free_days_map.png).")
    ...
    out_path = args.out or (OUTPUT_DIR / "pnw_frost_free_days_map.png")
