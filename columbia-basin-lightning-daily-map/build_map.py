...
def build_map(region_key, lightning_path, output_path):
    ...
    # box, and individual regions crop tighter than that shared box. Each
    # kept flash's opacity is scaled by edge_fade() so ones near the
    # boundary taper off instead of stopping at a hard line -- see
    # EDGE_FADE_FRAC above.
    lons, lats, alphas = [], [], []
    for flash in flashes:
        ...
        lons.append(lon)
        lats.append(lat)
        alphas.append(0.55 * fade)

    total_in_region = len(lons)
    if lons:
        ax.scatter(lons, lats, transform=pc, s=10, color=FLASH_COLOR, alpha=alphas,
