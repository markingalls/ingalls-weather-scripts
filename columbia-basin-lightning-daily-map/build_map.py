...
def build_map(region_key, lightning_path, output_path):
    ...
    # Projected to this axes' own native coordinates and checked against
    # its real xlim/ylim (see the native_xlim/native_ylim note above) --
    # not an approximate lat/lon box -- so flashes fill the frame all the
    # way to its true edge.
    ...
    lons, lats = all_lons[in_frame].tolist(), all_lats[in_frame].tolist()
