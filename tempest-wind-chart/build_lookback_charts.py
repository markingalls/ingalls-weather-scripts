"""
Fetches and renders a wind chart for each of the past N station-local
calendar days (not including today) -- a "look back" companion to the
always-current today chart (build_chart.py with no flags), the same idea
as tempest-temp-chart's own build_lookback_charts.py.
...
behavior (day boundary handling, gap-breaking, peak-gust marking, etc.)
always matches the today chart -- this script only adds the loop over
dates and the --no-current-conditions flag (there's no "current" reading
to highlight on a complete, past day).

On the droplet, deploy/publish_wind_daily.py maintains the actual
published 5-day archive (hermiston_wind_day{1..5}.png) going forward, one
day at a time via nightly rotation -- this script is for local testing,
or for backfilling all 5 days immediately after first deploying rather
than waiting 5 days for the archive to fill in naturally (copy its 5
output PNGs into /var/www/images as hermiston_wind_day{1..5}.png, oldest
as day5).
...
def main():
    ...
    for days_back in range(1, args.days + 1):
        ...
        obs_path = os.path.join(args.output_dir, f"tempest_wind_obs_{target_date}.json")
        chart_path = os.path.join(args.output_dir, f"tempest_wind_chart_{target_date}.png")
