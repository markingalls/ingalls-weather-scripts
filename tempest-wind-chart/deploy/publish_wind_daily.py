...
published as hermiston_wind_day{1..5}.png -- a look-back companion to the
always-current hermiston_wind.png publish_wind.py maintains every 5
minutes.
...
not still in progress -- there's no "current" reading to highlight).

Fetches into tempest_wind_obs_daily.json, deliberately not the plain
tempest_wind_obs.json publish_wind.py's own every-5-minutes tick reads
and writes -- these two cron jobs run independently (separate lock
files) and could tick concurrently, so sharing one file would risk one
script reading the other's half-written data.

An flock-based lock (state/daily_run.lock, distinct from publish_wind.py's
...
second pass concurrently -- and doesn't contend with publish_wind.py's
...
N_DAYS = 5
OUTPUT_BASE = "hermiston_wind"

BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # tempest-wind-chart/
...
def main():
    ...
    try:
        ...
        obs_path = os.path.join(BASE_DIR, "tempest_wind_obs_daily.json")
