...
for a WeatherFlow Tempest station and writes tempest_wind_obs.json -- wind
speed, gust, and direction, the same station/day-boundary handling as
../tempest-temp-chart/fetch_tempest.py's temperature fetch, just reading
different obs_st fields.
...
WIND_AVG_MS_INDEX = 2
WIND_GUST_MS_INDEX = 3
WIND_DIR_DEG_INDEX = 4
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", default="tempest_wind_obs.json")
    args = ap.parse_args()
    ...
    for row in obs_resp.get("obs") or []:
        epoch, wind_avg_ms = row[0], row[WIND_AVG_MS_INDEX]
        wind_gust_ms, wind_dir_deg = row[WIND_GUST_MS_INDEX], row[WIND_DIR_DEG_INDEX]
        if epoch is None or wind_avg_ms is None:
            continue
        local_time = datetime.fromtimestamp(epoch, tz)
        obs = {
            "time": local_time.isoformat(),
            "wind_speed_mph": round(ms_to_mph(wind_avg_ms), 1),
        }
        if wind_gust_ms is not None:
            obs["wind_gust_mph"] = round(ms_to_mph(wind_gust_ms), 1)
        if wind_dir_deg is not None:
            obs["wind_dir_deg"] = round(wind_dir_deg)
