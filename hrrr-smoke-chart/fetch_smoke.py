...
# Regions -- 5 cities each, for build_chart.py's --region to plot as one
# multi-line chart per region/variable combination. Coordinates reused
# from other projects' city lists in this repo where a matching label
# already existed there (Tri-Cities from ../tri-cities-7day-forecast/,
# the rest from ../wildcad-fires-map/ and ../columbia-basin-*-map/'s
# CITIES lists), rather than re-deriving new ones, so the same place name
# always means the same point across the repo.
REGIONS = {
    "columbia_basin": {
        "title": "Columbia Basin",
        "locations": [
            {"label": "Tri-Cities", "lat": 46.2647, "lon": -119.1189},
            {"label": "Hermiston", "lat": 45.8404, "lon": -119.2895},
            {"label": "Walla Walla", "lat": 46.0646, "lon": -118.3430},
            {"label": "Yakima", "lat": 46.6021, "lon": -120.5059},
            {"label": "Moses Lake", "lat": 47.1301, "lon": -119.2781},
        ],
    },
    "willamette": {
        "title": "Willamette Valley",
        "locations": [
            {"label": "Portland", "lat": 45.5152, "lon": -122.6765},
            {"label": "Hillsboro", "lat": 45.5229, "lon": -122.9898},
            {"label": "Salem", "lat": 44.9429, "lon": -123.0351},
            {"label": "Eugene", "lat": 44.0521, "lon": -123.0868},
            {"label": "Gresham", "lat": 45.5001, "lon": -122.4302},
        ],
    },
    "puget_sound": {
        "title": "Puget Sound",
        "locations": [
            ...
            {"label": "Tacoma", "lat": 47.2529, "lon": -122.4443},
            {"label": "Everett", "lat": 47.9790, "lon": -122.2021},
            {"label": "Bellevue", "lat": 47.6101, "lon": -122.2015},
            {"label": "Bellingham", "lat": 48.7519, "lon": -122.4443},
        ],
    },
}

# Default fetch is the union of every region's cities (15 total, all
# distinct) -- one HRRR pass feeds every region/variable chart
# build_chart.py can render, rather than fetching per region. --locations
# still overrides this entirely for an ad hoc single-point fetch.
DEFAULT_LOCATIONS = [loc for region in REGIONS.values() for loc in region["locations"]]
